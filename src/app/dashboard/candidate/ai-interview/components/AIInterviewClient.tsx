...
export function AIInterviewClient({ jobContext }: AIInterviewClientProps) {
      ...
      try {
        ...
        mediaRecorderRef.current.onerror = (event: Event) => {
          ...
          if (event instanceof DOMException) {
            errorMsg = `Video recording error: ${event.name} - ${event.message}.`;
          }
    ...
    if (mediaRecorderRef.current) {
      if (mediaRecorderRef.current.state === "recording" || mediaRecorderRef.current.state === "paused") {
        ...
        try {
          ...
          console.warn("CleanupResources: Error stopping media recorder:", e);
        }
      } else {
  ...
  const handleFinishInterview = useCallback(async () => {
    ...
    if (conversation && conversation.status === 'connected') {
      try {
        ...
        console.error("handleFinishInterview: Error ending EL session:", e);
      }
    }
  ...
  }, [submitForFinalFeedback, toast, cleanupResources, resetFullInterview, conversation]);

  const handleElevenError = useCallback((error: Error, context: string = "ElevenLabs") => {
    if (isProcessingErrorRef.current) {
      console.warn("handleElevenError: Already processing an error. Skipping.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (videoPreviewRef.current) {
          ...
          previewStreamRef.current = videoStream;
      ...
      // Request audio + video permission for recording
      ...
      if (error.name === "NotFoundError" || error.name === "DevicesNotFoundError") desc = "No camera/mic found.";
        ...
        desc = "Camera/Microphone permission denied.";
        setCameraPermission(false);
        setMicPermission(false);
      }
    ...
    <Card className="shadow-xl overflow-hidden min-h-[500px] md:min-h-[600px] flex flex-col">
      <CardContent className="flex-grow p-0 md:grid md:grid-cols-3 md:gap-0">
          ...
          <div className="w-full max-w-2xl aspect-video rounded-md shadow-lg bg-black relative">
            ...
            <div className="absolute top-0 left-0 right-0 p-2 md:p-3 bg-black/60 backdrop-blur-sm text-white rounded-t-md z-20 shadow-lg flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                ...
                <div className={cn('ai-speaking-orb', isSpeaking && 'speaking')} />
              </div>
              <div className="flex items-center gap-2 md:gap-3">
                {(stage === 'interviewing' && status === "connected" && !isSpeaking) && (
                  <div className="flex items-center text-xs text-white animate-pulse">
                    <Mic className="h-3 w-3 md:h-4 md:w-4 mr-1" /> Listening...
                  </div>
                )}
                <span className="text-xs">Status: {status || "Initializing..."}</span>
              </div>
            </div>
        ...
        <div className="md:col-span-1 p-4 flex flex-col border-l border-border/30 bg-background/90 md:bg-transparent">
          <div className="mb-4">
            {(stage === "interviewing" && status === 'connected') && (
          ...
          <div className="flex-grow overflow-y-auto space-y-2 pr-1 max-h-[300px] md:max-h-none">
            ...
            {conversationMessages.length === 0 && stage === 'interviewing' && !isSpeaking && (
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-xl flex items-center">
            <CheckCircle className="mr-2 h-6 w-6 text-green-500" /> Interview Analysis Complete
          </CardTitle>
        ...
        <CardContent className="space-y-6">
            ...
            <Card className="shadow-md">
              <CardHeader>
                <CardTitle className="text-lg flex items-center">
                  <Film className="mr-2 h-5 w-5 text-primary" /> Review Your Interview
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card className="shadow-md">
            <CardHeader>
              <CardTitle className="text-lg flex items-center">
                <MessageSquareIcon className="mr-2 h-5 w-5 text-primary"/>Full Conversation Transcript
              </CardTitle>
            </CardHeader>
            <CardContent>
